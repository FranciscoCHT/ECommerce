$(document).ready(function () {
    $(".table-button-del").on('submit', '.form-eliminar', function () {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                if (respuesta.mensaje == "ok") {
                    $(".table-button-del").DataTable().row(form.parents('tr')).remove().draw();
                    ...
                    $(".table-button-del").DataTable().row(rowActual).cell(rowActual.index(), 3).data("Inactivo").draw(); //Seteado a estado desactivado local en la tabla y redibujada.
                    ...
                    var rowActual = form.parents('tr');
                    $(".table-button-del").DataTable().row(rowActual).cell(rowActual.index(), 2).data("Inactiva").draw(); //Seteado a estado desactivado local en la tabla y redibujada.
